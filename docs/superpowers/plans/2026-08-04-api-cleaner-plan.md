# api-cleaner Skill 实现计划

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 创建一个纯 markdown skill（api-cleaner/SKILL.md），消费 api-finder 的输出，对每个外部接口独立重分析源码，应用严格排除规则去除误报。

**Architecture:** 单文件 skill（SKILL.md），采用"统一初始化 → 逐接口完整分析 → 最终输出"的流程。复用 api-finder 的分析范围确定逻辑和双方法文件检查规则，复用 arch.md 的架构分析结果，独立分析参数、调用关系和测试/冗余判断。

**Tech Stack:** 纯 markdown skill 文档，运行时依赖 Bash（find/grep/ls/mkdir）、Read、Write 工具，可选依赖 codegraph_explore 和 ast_grep_search MCP 工具。

---

### Task 1: 创建目录和 SKILL.md 骨架

**Files:**
- Create: `api-cleaner/SKILL.md`

- [ ] **Step 1: 创建 api-cleaner 目录**

```bash
mkdir -p /home/admin/cc/wksp/siakam_security_skills/api_finder/api-cleaner
```

- [ ] **Step 2: 写入 frontmatter 和概述部分**
...
---
name: api-cleaner
description: Use when the user invokes /api-cleaner or asks to remove false positives from external API interface analysis results. Consumes api-finder output (api.json, summary.md, arch.md), independently re-analyzes source code for each interface, and applies stricter exclusion rules (internal caller check, single-integer parameter exclusion, channel read direction verification). For embedded/low-level C codebases.
---
...
git commit -m "feat(api-cleaner): add SKILL.md skeleton with frontmatter and overview
...
**Files:**
- Modify: `api-cleaner/SKILL.md` (追加)
...
```bash
git add api-cleaner/SKILL.md
git commit -m "feat(api-cleaner): add initialization and scope determination chapter
...
搜索函数体代码，未发现任何信道读取相关的 API 调用（信道 API 列表见 2.4 节）。如果既无入参也无信道读取 → **排除**。
...
```bash
git add api-cleaner/SKILL.md
git commit -m "feat(api-cleaner): add per-interface analysis core chapter
...
### Task 4: 写入最终输出、约束和权限章节

**Files:**
- Modify: `api-cleaner/SKILL.md` (追加)

- [ ] **Step 1: 追加最终输出、约束和权限章节**
...
```bash
git add api-cleaner/SKILL.md
git commit -m "feat(api-cleaner): add output, constraints, permissions and examples
...
### Task 5: 设计审查 — 对照 spec 检查完整性

**Files:**
- Read: `docs/superpowers/specs/2026-08-04-api-cleaner-design.md`
- Read: `api-cleaner/SKILL.md`

- [ ] **Step 1: 对照 spec 逐项检查**

读取 spec 和 SKILL.md，逐项核对：

1. 概述与核心原则 → 对应 SKILL.md 概述 + 核心原则 ✓
2. 一、整体架构与数据流 → 目录结构在"使用方法"和"输出"中体现，数据流在概述中说明 ✓
3. 二、初始化流程 Step 1-6 → 对应 SKILL.md 一、Step 1-6 ✓
4. 三、逐接口分析 3.1-3.6 → 对应 SKILL.md 二、2.1-2.7 ✓
5. 四、最终输出 → 对应 SKILL.md 四、4.1-4.3 ✓
6. 五、约束规则 → 对应 SKILL.md 五、约束规则 ✓
7. 六、权限申请 → 对应 SKILL.md 六、权限申请 ✓

特别检查 spec 中的关键要求：
- [ ] 双方法文件检查（禁止 test -f）→ SKILL.md 一、Step 3 + 五、规则6
- [ ] 复用 arch.md → SKILL.md 一、Step 3 + 二、2.4.d
- [ ] 独立分析范围确定 → SKILL.md 一、Step 2
- [ ] 逐接口保存状态 → SKILL.md 二、开头 + 二、2.7
- [ ] 内部调用者排除 → SKILL.md 二、2.4.a
- [ ] 单整数参数排除 → SKILL.md 二、2.4.c
- [ ] 整数内存地址判断（类型+上下文）→ SKILL.md 二、2.4.c
- [ ] 信道读方向区分 → SKILL.md 二、2.5.a
- [ ] 信道单整数读取排除 → SKILL.md 二、2.5.b
- [ ] 通信边界关联验证 → SKILL.md 二、2.4.d
- [ ] 误报优先原则 → SKILL.md 核心原则2 + 二、2.6
- [ ] reason 必须具体 → SKILL.md 二、2.7
- [ ] 全部中文 → 全文中文 ✓
- [ ] 主 agent 完成 → 五、规则1 ✓

- [ ] **Step 2: 如有遗漏，补充修改后提交**

如发现遗漏，使用 Edit 工具修改 SKILL.md 后提交：

```bash
git add api-cleaner/SKILL.md
git commit -m "fix(api-cleaner): address spec coverage gaps from design review
...
# 如无修改，做一个验证性提交
git add api-cleaner/SKILL.md
git commit -m "chore(api-cleaner): verify SKILL.md completeness against design spec
