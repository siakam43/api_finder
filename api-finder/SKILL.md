...
3. 写入两份文件：
   - 将 fate = "inherited" 的条目（仅 name + file 字段）写入
     <project_dir>/.ethunter_out/api-finder/tmp/inherited_apis.json，格式与 old_api.json 一致，
     供第五节和后续阶段使用。
   - 将全部条目（含 inherited 和 eliminated）写入
     ...
     其中 file 保持 old_api.json 中的原始值。reason 必须写明确凿证据，不使用模糊描述。
...
逐条审查和分析范围校验全部完成后，将 old_api 继承接口中被淘汰的结果回写到 old_api_fate.json：
...
├── 确认存在 → 读取 old_api_fate.json，生成附节并追加到 finder_summary.md 末尾
...
若 Z = 0 或 W = 0，对应小节省略不输出。若 Y = 0，省去"淘汰接口详述"小节，仅保留开头统计。
