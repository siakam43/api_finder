...
- **关键特征：** 搜索该函数在本项目中的所有引用点，不应存在普通函数调用
  （任何 `函数名(args)` 形式的调用都意味着有内部调用者，该函数不是纯外部接口）
- 函数的参数来自外部不可信源
...
3. **对找到的每个候选接口，执行 LLM 深度验证：**
   ...
   **验证结论（按顺序逐一检查，命中任意排除条件即停止）：**
   - 内部函数间的数据传递（参数来自本项目其他函数计算的结果）→ 排除
   - 无法与 arch.md 中的任何通信边界关联、无法判断 → **排除**（误报优先原则）
   - 能与 arch.md 中的通信边界明确关联 → 保留为外部接口

   **注意：形式一的内部调用排除在分析方法 b 步骤中已经完成，不需要在本验证步骤中重复。**
   形式二（信道读取）的函数可能被项目内部代码调用，这不影响其外部接口属性。
   关键是判断数据来源是否为外部，而非函数是否被内部调用。
