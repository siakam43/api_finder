...
3. 对找到的每个函数（作为候选接口）：
   ...
   **重要：** `file` 字段必须填写**函数定义**所在的文件绝对路径。通过注册数组/结构体定义或注册函数调用点找到候选函数后，必须以函数名为关键词在 scope_files 中搜索函数定义位置（优先 codegraph_explore；否则用 grep 定位函数名所在行，再 Read 读取匹配行前后各 3 行上下文，确认是否存在函数体定义——需有完整签名 + `{` 允许换行、不以 `;` 结尾、不在注释中），将定义所在文件路径填入 `file`，而非注册点/注册数组所在文件路径。
...
1. **函数定义验证：** 在 `file` 指向的文件中，用 grep 定位函数名所在行，再 Read 读取匹配行前后各 3 行上下文，确认是否存在函数体定义（需满足：完整签名 + `{` 允许换行、不以 `;` 结尾、不在注释中）。
   - 包含函数定义 → 通过，`file` 不变
   - 不包含函数定义 → 用同样方法在全部 scope_files 中搜索该函数定义
